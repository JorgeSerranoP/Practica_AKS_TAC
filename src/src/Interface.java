...
public class Interface {
	...
	/*
	 * private BigInteger BigIntSqrt() { BigInteger TWO = BigInteger.valueOf(2);
	 * BigInteger rtn = toTest.divide(TWO);
	 * 
	 * BigInteger curr = (n.divide(rtn).add(rtn)).divide(TWO); BigInteger next =
	 * (n.divide(curr).add(curr)).divide(TWO);
	 * while(curr.subtract(next).abs().compareTo(BigInteger.ONE) != 0) { curr =
	 * next; next = (n.divide(curr).add(curr)).divide(TWO); } //Compute result =
	 * ((n/g) + g)/2. Let g be the result just computed. //Repeat step 2 until the
	 * last two results obtained are the same.
	 * 
	 * return curr; }
	 */
