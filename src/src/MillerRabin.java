...
public class MillerRabin extends Thread {
	...
	// taken from:
	/*
	 * Miller-Rabin wikipedia Pomerance, C.; Selfridge, J. L. & Wagstaff, S. S., Jr.
	 * (1980), "The pseudoprimes to 25á109", Mathematics of Computation 35 (151):
	 * 1003Ð1026, doi:10.2307/2006210 Jaeschke, Gerhard (1993),
	 * "On strong pseudoprimes to several bases", Mathematics of Computation 61
	 * (204): 915Ð926, doi:10.2307/2153262
	 */
	...
	private boolean ThreadPrimalityTest() {
		...
		MillerRabinThread MRT = new MillerRabinThread(m_prime, m_2factor, m_oddfactor, newsteps, BigInteger.ONE,
				portion);
		MillerRabinThread MRT1 = new MillerRabinThread(m_prime, m_2factor, m_oddfactor, newsteps, portion,
				portion.multiply(TWO));
		MillerRabinThread MRT2 = new MillerRabinThread(m_prime, m_2factor, m_oddfactor, newsteps, portion.multiply(TWO),
				portion.multiply(new BigInteger("3")));
		MillerRabinThread MRT3 = new MillerRabinThread(m_prime, m_2factor, m_oddfactor, newsteps,
				portion.multiply(new BigInteger("3")), m_prime);
		MRT.start();
		MRT1.start();
		MRT2.start();
		MRT3.start();
		...
		do {
			if (!MRT.isAlive() && !MRT.GetIsPrime() || !MRT1.isAlive() && !MRT1.GetIsPrime()
					|| !MRT2.isAlive() && !MRT2.GetIsPrime() || !MRT3.isAlive() && !MRT3.GetIsPrime()) {
				MRT.stop();
				MRT1.stop();
				MRT2.stop();
				MRT3.stop();
	...
	private BigInteger GetRandBigInt() {
		 ...
		 * 
		 * Random rand = new Random(); int numdigits =
		 * rand.nextInt(m_prime.bitLength()/8); for(int i = 0; i < numdigits; i++)
		 * randBigIntstr += rand.nextInt(10);
		 * 
		 * 
		 * BigInteger rtn = new BigInteger(randBigIntstr); return rtn;
		 */
	}
